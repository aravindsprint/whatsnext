...
- Two-way WhatsApp chat synced to ERPNext documents (Customers, Contacts, Leads, etc.)
...
- Event-driven notifications — fire a WhatsApp message automatically on any
  doctype's Submit / Update / Change via the **WhatsApp Notification** doctype
...
- Dashboard: message queue, delivery status donut, weekly trend, top templates,
  recent activity — modeled after the WhatsApp Hub reference design
...
```bash
cd ~/frappe-bench
bench get-app whatsnext /path/to/whatsnext
bench --site <site> install-app whatsnext
```
